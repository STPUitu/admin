...
- **GET actions** → JSONP via `<script>` tag (lihat bahagian 2)
- **POST actions** (`updateStatus`, `updateStatusBatch`) → `fetch()` biasa
...
### POST actions (guna `fetch` biasa):
| Action | Keterangan |
|--------|------------|
| `updateStatus` | Kemaskini status 1 rekod |
| `updateStatusBatch` | Kemaskini status berbilang rekod sekaligus |

POST tidak ada masalah CORS kerana GAS menerima POST request dari mana-mana origin.
